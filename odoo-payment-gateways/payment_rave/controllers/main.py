    ...
    def rave_verify_charge(self, **post):
        ...
        if tx.type == "form_save" and tx.partner_id:
            payment_token_id = (
                ...
                .sudo()
                .create(
                    {"acquirer_id": tx.acquirer_id.id, "partner_id": tx.partner_id.id,}
                )
            )
            ...
            response = tx._rave_verify_charge(data)
        _logger.info(
            "Rave: entering form_feedback with post data %s", pprint.pformat(response)
        )
        if response:
            request.env["payment.transaction"].sudo().with_context(
                lang=None
            ).form_feedback(response, "rave")

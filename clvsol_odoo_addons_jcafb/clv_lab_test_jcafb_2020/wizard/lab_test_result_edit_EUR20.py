...
class LabTestResultEditEUR20(models.TransientModel):
    _description = 'Lab Test Result Edit (EUR20)'
    _name = 'clv.lab_test.result.edit_eur20'
    ...
    def _default_EUR20_data_entrada_material(self):
        return self._get_default('EUR20', 'EUR20-01-01')
    EUR20_data_entrada_material = fields.Date(
        'Data de Entrada do Material', readonly=False, default=_default_EUR20_data_entrada_material
    )

    def _write_EUR20_data_entrada_material(self):
        self._set_result('EUR20', 'EUR20-01-01', self.EUR20_data_entrada_material)

    def _default_EUR20_liberacao_resultado(self):
        return self._get_default('EUR20', 'EUR20-01-02')
    EUR20_liberacao_resultado = fields.Date(
        'Liberação do Resultado', readonly=False, default=_default_EUR20_liberacao_resultado
    )

    def _write_EUR20_liberacao_resultado(self):
        self._set_result('EUR20', 'EUR20-01-02', self.EUR20_liberacao_resultado)

    def _default_EUR20_examinador(self):
        employee_model = self.env['hr.employee']
        code = self._get_default('EUR20', 'EUR20-01-03')
        if code is not False:
            ...
            return False
    EUR20_examinador = fields.Many2one(
        'hr.employee',
        string='Examinador',
        readonly=False,
        default=_default_EUR20_examinador
    )

    def _write_EUR20_examinador(self):
        if self.EUR20_examinador.name is not False:
            self._set_result(
                'EUR20', 'EUR20-01-03',
                self.EUR20_examinador.name + ' [' + self.EUR20_examinador.code + ']'
            )
        else:
            self._set_result('EUR20', 'EUR20-01-03', False)
    ...
    def do_result_updt_EUR20(self):

        self._write_EUR20_data_entrada_material()
        self._write_EUR20_liberacao_resultado()
        self._write_EUR20_examinador()

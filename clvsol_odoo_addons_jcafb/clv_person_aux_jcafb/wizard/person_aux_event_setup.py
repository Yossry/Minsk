...
class PersonAuxEventSetUp(models.TransientModel):
    _description = 'Person (Aux) Event Setup'
    _name = 'clv.person_aux.event_setup'

    def _default_person_aux_ids(self):
        return self._context.get('active_ids')
    person_aux_ids = fields.Many2many(
        comodel_name='clv.person_aux',
        relation='clv_person_aux_event_setup_rel',
        string='Persons (Aux)',
    ...
    event_ids = fields.Many2many(
        ...
        relation='clv_person_aux_event_setup_event_rel',
        string='Events'
    )

    document_type_ids = fields.Many2many(
        comodel_name='clv.document.type',
        relation='clv_person_aux_event_setup_document_type_rel',

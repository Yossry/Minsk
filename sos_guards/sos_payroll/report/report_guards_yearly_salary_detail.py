...
class guards_yearly_salary_report(report_sxw.rml_parse):

	def __init__(self, cr, uid, name, context):
		super(guards_yearly_salary_report, self).__init__(cr, uid, name, context)
	...
	def cal_monthly_amt(self, form, emp_id):
		...
		self.cr.execute('''SELECT rc.code, pl.name, sum(pl.total), \
			to_char(date_to,'mm-yyyy') as to_date  FROM guards_payslip_line as pl \
...
report_sxw.report_sxw('report.guards.salary.detail.byyear', 'guards.yearly.salary.detail', 'sos/report/report_guards_yearly_salary_detail.rml', parser=guards_yearly_salary_report, header='internal landscape')

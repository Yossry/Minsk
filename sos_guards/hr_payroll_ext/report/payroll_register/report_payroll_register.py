...
class report_payroll_summary(report_sxw.rml_parse):
	...
	# Most of this function (except at the end) is copied verbatim from the Pay Slip Details Report #
	def get_details_by_rule_category(self, obj):
		...
		# Choose only the categories (or rules) that we want to show in the report.  #
		reg_line = {
			'name': '',
			'id_no': '',
			'salary': 0,
		...
		if ids:
			...
			# Make adjustments to subtract from the parent category's total the amount of individual rules that we show separately on the sheet. #
...
class report_payslips(report_sxw.rml_parse):
	...
	def get_payslip_accruals(self, contract_ids, dToday):
		...
		for c in contract_obj.browse(contract_ids):
			policy = accrual_policy_obj.get_latest_policy(c.policy_group_id, dToday)
			if policy is None:
				continue
		...
		for slip in payslips:
			...
			if len(accruals) > 0:
				for accrual_id, code in accruals:
					balance = accrual_obj.get_balance([accrual_id], slip.employee_id.id, slip.date_to)

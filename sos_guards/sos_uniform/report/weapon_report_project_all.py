...
class ReportWeaponProjectAll(models.AbstractModel):
	_name = 'report.sos_uniform.report_weaponprojectall'
	_description = 'All Project Weapon Report'
	...
	def _get_report_values(self, docids, data=None):
		...
		for project in projects:
			self.env.cr.execute("select count(*) as project_guards from hr_employee e, hr_guard g where g.id = e.guard_id and g.project_id = %s and current =True and is_guard = True"%project.id)
			p = self.env.cr.dictfetchall()[0]
			project_guards = p['project_guards']
		
			demands = self.env['sos.weapon.demand'].search([('project_id', '=', project.id),('date', '>=', date_from), ('date', '<=', date_to),('state', '<>', 'reject')])
		...
		report = self.env['ir.actions.report']._get_report_from_name('sos_uniform.report_weaponprojectall')
		return {
			"doc_ids": self._ids,

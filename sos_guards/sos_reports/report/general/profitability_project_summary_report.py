...
class ReportProfitabilityProjectSummary(models.AbstractModel):
	_name = 'report.sos_reports.report_profitabilitysummaryproject'
	...
	def render_html(self, data=None):
		...
		for project in projects:		
			self.env.cr.execute("select sum(amount_untaxed) as amount_untaxed from account_invoice where project_id = %s and journal_id = %s \
				and date_invoice >= %s and date_invoice <= %s and state in ('open','paid') and inv_type != 'credit'",(project.id,1,date_from,date_to))
			invoiced_data = self.env.cr.dictfetchall()[0]	
			invoiced = int(invoiced_data['amount_untaxed'] or 0)
			
			self.env.cr.execute("select sum(amount_untaxed) as amount_total from account_invoice where project_id = %s and journal_id = %s \
				and date_invoice >= %s and date_invoice <= %s and state in ('open','paid') and inv_type ='credit'",(project.id,1,date_from,date_to))
			...
			sql = """select  sum(gpl.total) as salary_expense from guards_payslip gp, guards_payslip_line gpl, sos_project p
					...
					and p.id = gp.project_id and p.id = %s""" % (date_from,date_to,project.id,)	
		...
		report = report_obj._get_report_from_name('sos_reports.report_profitabilitysummaryproject')
		docargs = {
			"doc_ids": self._ids,
		...
		return report_obj.render('sos_reports.report_profitabilitysummaryproject', docargs)
